// 浏览器端 AI 直连（与后端 services/aiService.js 的提示词保持同步）
// 说明：服务模式由后端调用 AI；浏览器本地模式由本模块直接从浏览器调用

import { dbGet } from '../db/browser-db.js';

async function aiConfig() {
  const cfg = (await dbGet('settings', 'ai'))?.value || {};
  return {
    apiKey: cfg.apiKey || '',
    model: cfg.model || 'deepseek-chat',
    baseUrl: (cfg.baseUrl || 'https://api.deepseek.com').replace(/\/+$/, ''),
    temperature: Number(cfg.temperature ?? 0.8)
  };
}

export async function hasApiKeyLocal() {
  return !!(await aiConfig()).apiKey;
}
...
export async function testConnectionLocal() {
...
export function buildExercisePrompt({ chapterTitle, knowledgePoints, major, counts, types, extra, mistakeHints }) {
  const typeLabels = { choice: '选择题', blank: '填空题', calc: '计算题', proof: '证明题' };
