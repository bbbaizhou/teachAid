...
| 📅 **我的课程表** | 周视图网格（周一~周日 × 节次），班级/地点/节次/周次录入，调课/停课一键标注，今日课程卡片，课前 10 分钟桌面通知提醒，移动端列表视图 |
| 🤖 **AI 应用**（统一模块） | 下设三个 AI 工具，一键切换： |
| 📖 **· 课程导入** | 选择「章节 + 学生专业 + 导入风格（生活案例 / 专业应用 / 问题悬念）」，AI 生成贴合专业的课堂导入文案，一键复制或存入备课资料 |
| 📝 **· 习题生成** | 基础级（课后作业）/ 进阶级（章节小测）/ 提高级（拔高训练）三档难度，题型可选（选择/填空/计算/证明），题量自由配置，生成题目 + 详细解析，导出 Word/Markdown，支持把高频易错题纳入本地题库 |
| 🏛️ **· 课程思政** | 为章节生成课程思政融入方案：思政元素挖掘、融入案例与素材、课堂融入设计、可口语化的课堂话术；支持按侧重主题（家国情怀/科学精神/辩证思维/文化自信/工匠精神）生成，结果可复制或存入备课资料 |
| 📂 **备课内容整理** | 按「课程 - 章节 - 知识点」三级目录组织教案/PPT/导入文案/思政设计，重点/难点/易错点/考研考点标签，关键词全文检索，附件上传，单章节一键打包导出 zip |
